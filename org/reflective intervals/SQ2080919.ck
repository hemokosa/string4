...
fun int snumber(int i, int n) {
    return sn[i%sn.cap()][n%sn[i%sn.cap()].cap()];
}
...
// violin1
fun void violin1(){
    int i1; int i2; int n; float d; float rt; 
    float pr0; float po0; float vf0; float vg0; float vol0; int on1; int on2;
...
fun void violin2(){
    ...
    for( 0 => n; n < nm; n++ ) {
        ...
        distance(ivol, n) => vol0;
        // reverbration
        revmix(irev, n) => r[1].gain;
        // first string
        if (Std.randf() > onoff1) { 0 => on1; 0 => on2; } 
        else { 1 => on1; if (Std.randf() > onoff2) { 0 => on2; } else { 1 => on2; } }
        if (Std.randf() > 0.0) 1.0 => rt; else ratio => rt;
        playString(s[1][0], f[1][i1]*rt, pr0, po0, vf0, vg0, vol0, on1);
        <<< "violin2-1:", rt >>>;
        // second string (double-stop)
        if (Std.randf() > 0.0) Std.rand2f(0.0, 3.0)::second => now;
        if (i1 == 0) 1 => i2;
        ...
        if (i1 == 3) 2 => i2;
        if (Std.randf() > 0.0) 1.0 => rt; else ratio => rt;
        playString(s[1][1], f[1][i2]*rt, pr0, po0, vf0, vg0, vol0, on1);
        <<< "violin2-2:", rt >>>;
        // advance time
        duration(idur, n) => d;
        d::second => now;
...
fun void viola(){
    ...
    for( 0 => n; n < nm; n++ ) {
        ...
        distance(ivol, n) => vol0;
        // reverbration
        revmix(irev, n) => r[2].gain;
        // first string
        if (Std.randf() > onoff1) { 0 => on1; 0 => on2; } 
        else { 1 => on1; if (Std.randf() > onoff2) { 0 => on2; } else { 1 => on2; } }
        if (Std.randf() > 0.0) 1.0 => rt; else ratio => rt;
        playString(s[2][0], f[2][i1]*rt, pr0, po0, vf0, vg0, vol0, on1);
        <<< "viola-1:", rt >>>;
        // second string (double-stop)
        if (Std.randf() > 0.0) Std.rand2f(0.0, 3.0)::second => now;
        if (i1 == 0) 1 => i2;
        ...
        if (i1 == 3) 2 => i2;
        if (Std.randf() > 0.0) 1.0 => rt; else ratio => rt;
        playString(s[2][1], f[2][i2]*rt, pr0, po0, vf0, vg0, vol0, on1);
        <<< "viola-2:", rt >>>;
        // advance time
        duration(idur, n) => d;
        d::second => now;
...
fun void cello(){
    ...
    for( 0 => n; n < nm; n++ ) {
        ...
        distance(ivol, n) => vol0;
        // reverbration
        revmix(irev, n) => r[3].gain;
        // first string
        if (Std.randf() > onoff1) { 0 => on1; 0 => on2; } 
        else { 1 => on1; if (Std.randf() > onoff2) { 0 => on2; } else { 1 => on2; } }
        if (Std.randf() > 0.0) 1.0 => rt; else ratio => rt;
        playString(s[3][0], f[3][i1]*rt, pr0, po0, vf0, vg0, vol0, on1);
        <<< "cello-1:", rt >>>;
        // second string (double-stop)
        if (Std.randf() > 0.0) Std.rand2f(0.0, 3.0)::second => now;
        if (i1 == 0) 1 => i2;
        ...
        if (i1 == 3) 2 => i2;
        if (Std.randf() > 0.0) 1.0 => rt; else ratio => rt;
        playString(s[3][1], f[3][i2]*rt, pr0, po0, vf0, vg0, vol0, on1);
        <<< "cello-2:", rt >>>;
        // advance time
        duration(idur, n) => d;
        d::second => now;
