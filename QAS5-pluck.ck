...
fun void play2(StifKarp s, float f, float pr, float po, float vf, float vg, float vol, int onoff) {
    if (f < 0.0) 0 => onoff;
    f => s.freq;
    pr => s.sustain;
    po => s.pickupPosition;
...
fun void instrument(int k){
    ...
    for( 0 => n; n < nm; n++ ) {
        ...
        // first string
        if (Std.randf() > radio0) 1.0 => rt; else ratio => rt;
        play2(s[k][0], f[k][i0]*rt, pr0, po0, vf0, vg0, vol0, on0);
        // second string (double-stop)
        Std.rand2f(0.0, 3.0)::second => now;
        if (Std.randf() > radio1) 1.0 => rt; else ratio => rt;
        play2(s[k][1], f[k][i1]*rt, pr0, po0, vf0, vg0, vol0, on1);
        // advance time
        duration(idur, n) => d;
        d::second => now;
        // stop playing
        play2(s[k][0], f[k][i0], 0.01, 0.01, 0.01, 0.01, 0.0, 0);
        play2(s[k][1], f[k][i1], 0.01, 0.01, 0.01, 0.01, 0.0, 0);
        Std.rand2f(0.0, 3.0)::second => now;
    }
}
